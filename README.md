...
**CSS: Casecading Style Sheets**을 사용하여 HTML 요소의 스타일과 레이아웃을 지정한다.<br>
...
4. 레이아웃: 렌더 트리의 각 요소들에 대해 위치와 크기를 계산하여 레이아웃(Layout) 단계를 수행한다. 이 단계에서 각 요소의 상대적인 배치와 크기가 결정된다.
   <br>
5. 페인트: 레이아웃된 요소들을 실제로 화면에 그리는 페인트(Paint) 단계. CSS를 사용하여 레이아웃된 요소들에 대해 필요한 그래픽 리소스를 로드하고, 색상, 배경, 그림자 등을 적용하여 화면에 픽셀로 그린다.
   <br>

...
React Immer는 불변성을 유지하면서 상태를 간편하게 업데이트할 수 있도록 도와주는 라이브러리! React에서 상태를 업데이트할 때는 불변성을 유지하기 위해 새로운 객체를 생성하거나 배열을 복사하는 등의 작업을 해야하지만 Immer를 통해 번거로운 작업 없이 원본 상태를 직접 수정하는 것처럼 보이는 코드를 작성할 수 있다.

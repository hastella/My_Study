...
stack과 queue 모두 일렬로 정렬된 데이터를 다루는 자료구조이다. <br>우선 ❗️**스택은 프링글스통**을 떠올려보자! Last In First Out으로, 가장 최근에 추가된 데이터가 가장 먼제 삭제된다. push와 pop을 통해 요소를 추가/ 삭제하며, top은 스택의 맨 위에 있는 데이터를 반환한다.<br>❗️**큐는 줄서는 사람들**을 생각하면 이해가 쉽다! First In First Out으로 가장 오래된 데이터부터 삭제된다. enqueue와 dequeue를 통해 요소를 추가/ 삭제하며, front는 맨 앞에 있는 데이터를 반환한다.
<br>
